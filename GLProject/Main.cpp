...
int main()
{
	...
	unsigned char* edata = stbi_load("textures/r.jpg", &ewidth, &eheight, &enrChannels, 0);
	if (edata) {
	...
	unsigned char* sdata = stbi_load("textures/ss.jpg", &swidth, &sheight, &snrChannels, 0);
	if (sdata) {
	...
	unsigned char* mdata = stbi_load("textures/moon.jpg", &mwidth, &mheight, &mnrChannels, 0);
	if (mdata) {
